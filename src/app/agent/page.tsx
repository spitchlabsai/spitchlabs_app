...
import { Upload, Search, FileText, Trash2, Play, Square, Users, Activity, AlertCircle, CheckCircle, Clock, Download } from 'lucide-react';
...
const AgenticDocumentDashboard = () => {
  ...
  const [knowledgeBase, setKnowledgeBase] = useState({ total_documents: 0, total_chunks: 0, documents: [] });
  ...
  const stopAgentSession = async (roomName) => {
    try {
      const response = await fetch(`${API_BASE}/agent/stop/${user.id}/${roomName}`, {
        method: 'POST',
      });
        ...
        <div className="max-w-7xl mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-2xl font-bold text-gray-900">Spitchlabs AI</h1>
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div className="bg-white rounded-2xl shadow-lg p-6 border border-gray-100">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-gray-900">{knowledgeBase.total_documents}</p>
              </div>
          ...
          <div className="bg-white rounded-2xl shadow-lg p-6 border border-gray-100">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-gray-600 text-sm font-medium">Knowledge Chunks</p>
                <p className="text-3xl font-bold text-gray-900">{knowledgeBase.total_chunks}</p>
              </div>
          ...
          <div className="bg-white rounded-2xl shadow-lg p-6 border border-gray-100">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-gray-900">{agentSessions.length}</p>
              </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-200 mb-8">
          ...
          <div className="p-6">
            ...
            {activeTab === 'documents' && (
              <div className="space-y-6">
                ...
                <div className="border-2 border-dashed border-gray-300 rounded-xl p-8 text-center bg-gray-50 hover:bg-gray-100 transition-colors">
                  ...
                  <p className="text-gray-600 mb-4">Support for PDF, DOCX, and DOC files</p>
                  <input
                    type="file"
                    accept=".pdf,.docx,.doc"
                  ...
                  <label
                    htmlFor="file-upload"
                    className={`inline-flex items-center px-6 py-3 border border-transparent rounded-lg shadow-sm text-sm font-medium text-white bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 cursor-pointer transition-all ${
                      isUploading ? 'opacity-50 cursor-not-allowed' : ''
                    }`}
                  >
                    {isUploading ? <Clock className="w-4 h-4 mr-2 animate-spin" /> : <Upload className="w-4 h-4 mr-2" />}
                    {isUploading ? 'Uploading...' : 'Choose File'}
                  </label>
                  {uploadStatus && (
                    <div className={`mt-4 p-3 rounded-lg ${uploadStatus.includes('Error') ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'}`}>
                      <div className="flex items-center justify-center">
                        {uploadStatus.includes('Error') ? <AlertCircle className="w-4 h-4 mr-2" /> : <CheckCircle className="w-4 h-4 mr-2" />}
                        {uploadStatus}
                      </div>
                    </div>
                  )}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                      {documents.map((doc) => (
                        <div key={doc.id} className="bg-gray-50 rounded-xl p-4 border border-gray-200 hover:shadow-md transition-shadow">
                          ...
                          <h4 className="font-medium text-gray-900 mb-1 truncate">{doc.filename}</h4>
                          <p className="text-sm text-gray-600">Chunks: {doc.chunk_count || 0}</p>
            ...
            {activeTab === 'search' && (
              <div className="space-y-6">
                <div>
                  <h3 className="text-lg font-medium text-gray-900 mb-4">Search Your Documents</h3>
                  <div className="flex space-x-4">
                    ...
                    >
                      {isSearching ? <Clock className="w-4 h-4 mr-2 animate-spin" /> : <Search className="w-4 h-4 mr-2" />}
                ...
                {searchResults.length > 0 && (
                  <div>
                    <h4 className="text-md font-medium text-gray-900 mb-4">Search Results ({searchResults.length})</h4>
                    <div className="space-y-4">
                      {searchResults.map((result, index) => (
                        <div key={index} className="bg-gray-50 rounded-xl p-4 border border-gray-200">
                          ...
                          <p className="text-gray-700 text-sm leading-relaxed">{result.chunk_text}</p>
                        </div>
                      ))}
                    </div>
                  </div>
            ...
            {activeTab === 'agent' && (
              <div className="space-y-6">
                <div>
                  <h3 className="text-lg font-medium text-gray-900 mb-4">Agent Sessions</h3>
                  <div className="bg-gray-50 rounded-xl p-6">
                    <h4 className="font-medium text-gray-900 mb-4">Start New Session</h4>
                    <div className="flex space-x-4">
                      <input
                        type="text"
                        value={roomName}
                  ...
                  {agentSessions.length === 0 ? (
                    ...
                    <div className="space-y-3">
                      {agentSessions.map((session, index) => (
                        <div key={index} className="bg-gray-50 rounded-xl p-4 border border-gray-200 flex items-center justify-between">
                          <div>
                            <h5 className="font-medium text-gray-900">{session.room_name}</h5>
